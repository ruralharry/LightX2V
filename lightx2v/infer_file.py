...
logger.add(f"{log_dir}/0722file.log", rotation="10 MB", retention="365 days", encoding="utf-8")
os.system("echo 0 > /data/docker/state.txt")
...
def main():
    ...
    signal_file="/data/docker/config.json"
    output_signal_file="/data/docker/output_video_path.txt"
    os.system("> %s"%signal_file)
    os.system("> %s"%output_signal_file)
    os.environ["output_signal_file"] = output_signal_file
    runner = init_runner(config)
    os.system("echo 1 > /data/docker/state.txt")
    while(1):
        while not os.path.getsize(signal_file)>0:
            time.sleep(0.2)
        try:
            with open(signal_file, 'r') as f:more_config = json.loads(f.read())
